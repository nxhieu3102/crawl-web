...
class ProductList(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if detail != None:
            ...
            results = Product.objects.filter(Q(Type = response_data['Type']) & ~Q(ShopName = response_data['ShopName']))
            results = results.annotate(similarity=TrigramSimilarity('ProductName', response_data['ProductName']),).filter(similarity__gte = 0.1).order_by('-similarity')[:20]
            ...
            for item in results:
                if item.similarity > shopmapping[item.ShopName]:
                    ...
                    shopresults[item.ShopName] = item.ProductLink
            ...
            for key in sorted_dict:
                ...
                if idx > 1:
                    ...
                    response_data['ProductRelevant'][key] = {
        ...
        if search != None:
            ...
            from django.contrib.postgres.search import TrigramSimilarity
            results = Product.objects.annotate(similarity=TrigramSimilarity('ProductName', search),).filter(similarity__gte=0.2).order_by('-similarity', '-SalePrice')
            dataList = []
            for item in results:
